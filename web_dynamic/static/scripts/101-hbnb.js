...
const w = window.$;
const url = 'http://localho:5001/api/v1/status/';
...
w('h2 span').click(function() {
    ...
    if (span.text() === 'hide') {
        ...
        w.ajax({
        url: `http://0.0.0.0:5001/api/v1/places/${$('div#place_id').text()}/reviews`,
        type: 'GET',
        dataType: 'json',
        success: function (data) {
          for (const review of data) {
            const template = `<div class="review">
                                <h3>${review.user_email}</h3>
                                <p>${review.text}</p>
                              </div>`;
            $('section.reviews').append(template);
          }
          span.text('hide');
        }
    });
